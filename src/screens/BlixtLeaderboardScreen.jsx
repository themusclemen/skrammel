...
import { fetchBlixtLeaderboard, fetchFriendsBlixtLeaderboard } from "../api/blixt.js";

export default function BlixtLeaderboardScreen({ user, onBack, onChallenge }) {
  ...
  useEffect(() => {
    setRows(null);
    setError(null);
    const load = scope === "friends" && user ? fetchFriendsBlixtLeaderboard(user.id) : fetchBlixtLeaderboard();
    load.then(setRows).catch((err) => setError(err.message));
  }, [scope, user]);

  return (
    <div style={styles.page}>
      <h2 style={{ margin: 0, color: T.accent }}>⚡ Topplista för Blixt</h2>
      ...
      {rows && rows.length === 0 && (
        <div style={{ color: T.muted }}>
          {scope === "friends" ? "Ingen av dina vänner har spelat en blixt-match än." : "Ingen har spelat en blixt-match än."}
        </div>
      )}
      ...
      <div style={styles.navRow}>
        <button onClick={onBack} style={styles.navButton}>Till Blixt</button>
      </div>
    </div>
  );
}
