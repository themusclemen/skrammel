...
// ändrats sen den här enheten senast öppnade matchens hub". Delas mellan
// Blixt och Skrammelpaj via en egen storage-key-prefix per spel.

export function loadSeenStatuses(storageKeyPrefix, userId) {
  try {
...
export function saveSeenStatuses(storageKeyPrefix, userId, challenges) {
  const map = loadSeenStatuses(storageKeyPrefix, userId);
  for (const c of challenges) map[c.id] = c.status;
  try {
    localStorage.setItem(storageKeyPrefix + userId, JSON.stringify(map));
  } catch {
